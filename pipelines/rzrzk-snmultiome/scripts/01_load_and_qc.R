...
library(future); library(Seurat); library(Signac); library(EnsDb.Mmusculus.v79); library(BSgenome.Mmusculus.UCSC.mm10); library(yaml)
...
generate_object <- function(sample_cfg) {
    ...
    input.10x <- Read10X_h5(sample_cfg$h5); rna.counts <- input.10x$`Gene Expression`
    ...
    obj[["ATAC"]] <- CreateChromatinAssay(counts=atac.counts, sep=c(":", "-"), genome=genome(annotations),
...
qc_and_filter <- function(obj) {
    DefaultAssay(obj) <- "ATAC"; obj <- NucleosomeSignal(obj); obj <- TSSEnrichment(obj); qc <- cfg$qc
    obj <- subset(x=obj, subset = nCount_ATAC < qc$nCount_ATAC_max & nCount_RNA < qc$nCount_RNA_max &
        nCount_ATAC > qc$nCount_ATAC_min & nCount_RNA > qc$nCount_RNA_min &
        nucleosome_signal < qc$nucleosome_signal_max & TSS.enrichment > qc$TSS_enrichment_min & percent.mt < qc$percent_mt_max)
    return(obj)
}
...
cat("Running QC and filtering ...\n"); wild <- qc_and_filter(wild); mutant <- qc_and_filter(mutant)
cat(sprintf("  Wild-type cells after QC: %d\n", ncol(wild))); cat(sprintf("  Mutant cells after QC:    %d\n", ncol(mutant)))
out_dir <- cfg$output_dir; dir.create(file.path(out_dir,"rds"), showWarnings=FALSE, recursive=TRUE); dir.create(file.path(out_dir,"logs"), showWarnings=FALSE, recursive=TRUE)
saveRDS(wild, file.path(out_dir,"rds","wild_qc.rds")); saveRDS(mutant, file.path(out_dir,"rds","mutant_qc.rds"))
cat("Step 1 complete.\n")