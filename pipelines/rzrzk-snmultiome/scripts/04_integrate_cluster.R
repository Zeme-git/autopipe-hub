...
wild <- readRDS(file.path(out_dir,"rds","wild_normalized.rds")); mutant <- readRDS(file.path(out_dir,"rds","mutant_normalized.rds"))
cat("Finding integration anchors (SCT) ...\n")
DefaultAssay(wild) <- "SCT"; DefaultAssay(mutant) <- "SCT"
obj.list <- list(wild, mutant); features <- SelectIntegrationFeatures(obj.list)
...
cat("PCA, UMAP, clustering ...\n")
npcs <- cfg$integration$npcs; DefaultAssay(combined) <- "integrated"
combined <- ScaleData(combined, verbose=FALSE); combined <- RunPCA(combined, npcs=npcs, verbose=FALSE)
...
res_col <- paste0("integrated_snn_res.", cfg$integration$cluster_resolution_coarse)
combined@meta.data$cl_genotype <- paste0(combined@meta.data[[res_col]], "_", combined@meta.data$kras_genotype)
