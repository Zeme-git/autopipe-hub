...
process_expression <- function(obj) {
    obj <- SCTransform(obj); DefaultAssay(obj) <- "RNA"
    obj <- NormalizeData(obj); obj <- FindVariableFeatures(obj, selection.method="vst", nfeatures=cfg$rna$nfeatures)
    return(obj)
}
process_accessibility <- function(obj) {
    for (a in c("ATAC","macs2")) { DefaultAssay(obj) <- a; obj <- FindTopFeatures(obj, min.cutoff="q0"); obj <- RunTFIDF(obj); obj <- RunSVD(obj) }
    return(obj)
}
