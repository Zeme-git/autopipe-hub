...
# =============================================================================
#  Reads the 10x Cell Ranger ARC outputs (h5 + ATAC fragments) for both
#  Wild-type (RZ) and Mutant (RZK) samples. Creates joint RNA + ATAC Seurat
#  objects, computes QC metrics, and filters cells.
# =============================================================================
...
# ---- Function: create Seurat multiome object ---------------------------------
generate_object <- function(sample_cfg) {
    ...
    obj[["ATAC"]] <- CreateChromatinAssay(
        ...
        fragments  = sample_cfg$fragments,
        min.cells  = 10,
...
# ---- Function: QC metrics + cell filtering -----------------------------------
qc_and_filter <- function(obj) {
    ...
    obj <- subset(
        x = obj,
        subset = nCount_ATAC < qc$nCount_ATAC_max &
                 nCount_RNA  < qc$nCount_RNA_max   &
                 nCount_ATAC > qc$nCount_ATAC_min  &
                 nCount_RNA  > qc$nCount_RNA_min   &
                 nucleosome_signal < qc$nucleosome_signal_max &
                 TSS.enrichment    > qc$TSS_enrichment_min    &
