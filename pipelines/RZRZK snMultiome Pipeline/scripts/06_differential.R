...
# =============================================================================
#  6-A. Global DEG: Mutant vs Wild_type (MAST)
#  6-B. Hallmark KRAS_SIGNALING_UP overlap via gprofiler2
#  6-C. DA peaks: Wnt7+ vs Lgr5+ in both Mutant and Wild-type
# =============================================================================
...
# ---- 6-A. DEG: Mutant vs Wild_type (MAST) -----------------------------------
cat("Running MAST DE: Mutant vs Wild_type ...\n")
DefaultAssay(RZ.RZK) <- "RNA"
DEG <- FindMarkers(RZ.RZK, group.by = "kras_genotype",
                   ident.1 = "Mutant", test.use = cfg$de$test_use)
write.csv(DEG, file.path(out_dir, "tables", "RZRZK_DEG_scMAST.csv"))

DEG_sig <- DEG[DEG$p_val_adj < cfg$de$padj_threshold &
               DEG$avg_log2FC > cfg$de$log2fc_threshold, ]
cat(sprintf("  Significant up-regulated DEGs: %d\n", nrow(DEG_sig)))

# ---- 6-B. Hallmark KRAS overlap ---------------------------------------------
...
human_orth <- gconvert(query = rownames(DEG_sig),
                       organism = "hsapiens", target = "ENSG")
overlap    <- intersect(human_orth$name, kras_up)
cat(sprintf("  KRAS-UP genes: %d | DEGs converted: %d | Overlap: %d\n",
            length(kras_up), length(human_orth$name), length(overlap)))

# ---- 6-C. DA peaks: Wnt7+ vs Lgr5+ -----------------------------------------
cat("Running DA peaks (LR test) ...\n")
DefaultAssay(RZ.RZK) <- "ATAC"
plan("multicore", workers = 1)   # LR test is single-threaded

da_args <- list(
    min.pct         = cfg$da$min_pct,
    logfc.threshold = cfg$da$logfc_threshold,
    test.use        = cfg$da$test_use,
    latent.vars     = cfg$da$latent_vars
)

# Wnt7+ vs Lgr5+ in Mutant
da_mut <- do.call(FindMarkers, c(
    list(object = RZ.RZK, ident.1 = "3_Mutant", ident.2 = "1_Mutant",
         group.by = "cl_genotype"),
    da_args
))

# Wnt7+ vs Lgr5+ in Wild-type
da_wt <- do.call(FindMarkers, c(
    list(object = RZ.RZK, ident.1 = "3_Wild_type", ident.2 = "1_Wild_type",
         group.by = "cl_genotype"),
    da_args
))
...
annotate_da <- function(obj, da_peaks) {
    ...
    da_peaks$gene         <- closest$gene_name
    da_peaks$gene_biotype <- closest$gene_biotype
