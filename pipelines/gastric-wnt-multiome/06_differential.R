...
library(future); library(Seurat); library(Signac); library(dplyr)
library(MAST); library(msigdbr); library(gprofiler2); library(yaml)
...
da_args <- list(min.pct=cfg$da$min_pct, logfc.threshold=cfg$da$logfc_threshold,
                test.use=cfg$da$test_use, latent.vars=cfg$da$latent_vars)
...
da_mut <- annotate_da(RZ.RZK, da_mut); da_wt <- annotate_da(RZ.RZK, da_wt)
