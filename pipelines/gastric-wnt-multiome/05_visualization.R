...
# Helper to safely get expression data (works for both v3 assay and v5 layers)
...
existing_clusters <- unique(RZ.RZK@meta.data$clusters)
existing_clusters <- existing_clusters[!is.na(existing_clusters)]
...
# Only plot marker genes that exist
...
# ===== Fig 3E: Wnt7b-expressing cell percentage =====
cat("  Fig 3E ...\n")
...
# ===== Fig 3H: Coverage / linkage plot for Wnt7b =====
cat("  Fig 3H ...\n")
tryCatch({
...
for(cl in 0:6) {
    ...
    tryCatch({
        ...
        wt_expr <- get_gene_expr(RZ.RZK, "Wnt7b")[wt_cells]
        mut_expr <- get_gene_expr(RZ.RZK, "Wnt7b")[mut_cells]
        res <- wilcox.test(wt_expr, mut_expr)
        stats_rows[[length(stats_rows)+1]] <- data.frame(
            comparison=sprintf("Cluster%d_WT_vs_Mut_Wnt7b_expr",cl), test="Wilcoxon",
            p_value=res$p.value, statistic=NA, stringsAsFactors=FALSE)
...
for(region_str in cfg$wnt7b_regions) {
    tryCatch({
        ...
        for(cl in 0:6) {
            wt_id <- paste0(cl,"_Wild_type"); mut_id <- paste0(cl,"_Mutant")
            if(!(wt_id %in% all_cl_genotypes) || !(mut_id %in% all_cl_genotypes)) next
            cells_m <- colnames(subset(RZ.RZK, subset=cl_genotype==mut_id))
            cells_w <- colnames(subset(RZ.RZK, subset=cl_genotype==wt_id))
            if(length(cells_m)<2 || length(cells_w)<2) next
            res <- t.test(region_counts[cells_m], region_counts[cells_w])
            stats_rows[[length(stats_rows)+1]] <- data.frame(
                comparison=sprintf("Cluster%d_Mut_vs_WT_%s",cl,region_str), test="t_test",
                p_value=res$p.value, statistic=res$statistic, stringsAsFactors=FALSE)
        }
...
if(length(existing_wnt) > 0) {
    ...
    pdf(file.path(fig_dir,"fig_S3B_Wnt_family_violin.pdf")); plot.new(); title("No Wnt genes found"); dev.off()
}
...
if(length(stats_rows) > 0) {
    ...
    stats_df <- data.frame(comparison=character(), test=character(), p_value=numeric(), statistic=numeric())
}
