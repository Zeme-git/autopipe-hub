...
plot_featureplot <- function(obj, gene) {
    ...
    ggplot(data=p$data, aes_string(x="UMAP_1",y="UMAP_2",fill=gene)) +
        ...
        scale_fill_gradientn(colours=brewer.pal(9,"YlOrRd")) + theme_void() +
...
plot_featureplot_v3 <- function(obj, gene, min_cut, max_cut) {
    ...
    p <- FeaturePlot(obj,features=gene,pt.size=1,min.cutoff=min_cut,max.cutoff=max_cut,order=TRUE)
    ggplot(data=p$data,aes_string(x="UMAP_1",y="UMAP_2",fill=gene)) +
        geom_point(shape=21,stroke=NA,color="black",alpha=1,size=2) +
        scale_fill_gradientn(colours=ht_custom_col,name="Expression\nLevel") +
        ggtitle(gene) + theme_void() + theme(plot.title=element_text(hjust=0.5,size=16))
}
...
p_3B <- DotPlot(RZ.RZK,features=cfg$marker_genes,dot.scale=10) + scale_colour_gradient2(low="dodgerblue3",mid="ghostwhite",high="firebrick",limits=c(-2.5,2.5)) + xlab("Markers")+ylab("Clusters")+coord_flip()+theme(axis.text=element_text(size=15),legend.position="top")
ggsave(p_3B,filename=file.path(fig_dir,"fig_3B_dotplot.pdf"),width=6,height=7)

cat("  Fig 3C ...\n")
p_3C <- dittoBarPlot(RZ.RZK,Idents(RZ.RZK),group.by="kras_genotype",x.reorder=c(2,1),var.labels.reorder=c(1,6,2,5,4,3,7),color.panel=cluster_col)
...
for (pair in cl_list) {
    ...
    for (j in pair) {
        ...
        per <- c(per,pct); wnt_counts <- c(wnt_counts,gc_n); non_wnt <- c(non_wnt,total-gc_n)
...
p_3E <- ggplot(df_3E,aes(Clusters,Percentage,fill=Clusters))+geom_bar(stat="identity")+scale_x_discrete(limits=c("1_Wild_type","1_Mutant","2_Wild_type","2_Mutant","0_Wild_type","0_Mutant","4_Wild_type","4_Mutant","5_Wild_type","5_Mutant","6_Wild_type","6_Mutant","3_Wild_type","3_Mutant"))+ylim(0,100)+theme_classic()+scale_fill_manual(values=my_col)+ggtitle("Wnt7b-expressing cells (%)")+theme(plot.title=element_text(hjust=0.5))+NoLegend()
...
lv <- c()
for (i in c(1,2,0,4,5,6,3)) { lv <- c(lv,sprintf("%s_Wild_type",i),sprintf("%s_Mutant",i)) }
...
for (cl in 0:6) {
    ...
    wt_expr <- RZ.RZK[["RNA"]]@data["Wnt7b",wt_cells]; mut_expr <- RZ.RZK[["RNA"]]@data["Wnt7b",mut_cells]
...
for (spec in cfg$featureplot_genes) {
    ...
    ggsave(p,filename=file.path(fig_dir,"S3A",sprintf("fig_S3A_%s.pdf",spec$gene)),width=6,height=6,dpi=300)
}

cat("  Fig S3B ...\n")
DefaultAssay(RZ.RZK) <- "RNA"
p_S3B <- VlnPlot(object=RZ.RZK,features=cfg$wnt_family_genes,group.by="kras_genotype")
