...
get_gene_expr <- function(obj, gene, assay_name = "RNA") {
    ...
    tryCatch({ return(GetAssayData(obj, slot = "data")[gene, ]) },
    error = function(e) { tryCatch({ return(obj[[assay_name]]@data[gene, ]) },
    error = function(e2) { return(LayerData(obj, assay = assay_name, layer = "data")[gene, ]) }) })
}
...
fig_dir <- file.path(out_dir,"figures"); stats_rows <- list()
...
p_3B <- DotPlot(RZ.RZK, features=existing_markers, dot.scale=10) +
    ...
    xlab("Markers") + ylab("Clusters") + coord_flip() + theme(axis.text=element_text(size=15), legend.position="top")
...
cl_list <- list(c("0_Wild_type","0_Mutant"),c("1_Wild_type","1_Mutant"),c("2_Wild_type","2_Mutant"),
                c("3_Wild_type","3_Mutant"),c("4_Wild_type","4_Mutant"),c("5_Wild_type","5_Mutant"),c("6_Wild_type","6_Mutant"))
pp <- c(); cl_vec <- c()
for(pair in cl_list) { wnt_counts <- c(); non_wnt <- c()
    for(j in pair) { if(!(j %in% all_cl_genotypes)) { pp<-c(pp,0); cl_vec<-c(cl_vec,j); wnt_counts<-c(wnt_counts,0); non_wnt<-c(non_wnt,0); next }
        sub_obj <- subset(RZ.RZK, subset=cl_genotype==j); counts <- get_gene_expr(sub_obj, "Wnt7b")
        gc_n <- sum(counts != 0); total <- length(counts); pct <- gc_n/total*100
        wnt_counts <- c(wnt_counts, gc_n); non_wnt <- c(non_wnt, total-gc_n); pp <- c(pp, pct); cl_vec <- c(cl_vec, j) }
    if(sum(wnt_counts)>0 && sum(non_wnt)>0) { tryCatch({ fisher.df <- matrix(c(wnt_counts, non_wnt), nrow=2)
        fisher_res <- fisher.test(fisher.df, alternative="less")
        stats_rows[[length(stats_rows)+1]] <- data.frame(comparison=paste(pair,collapse=" vs "), test="Fisher_exact", p_value=fisher_res$p.value, statistic=fisher_res$estimate, stringsAsFactors=FALSE)
    }, error=function(e) cat(sprintf("    Fisher skipped: %s\n", e$message))) } }
df_3E <- data.frame(Clusters=cl_vec, Percentage=pp); df_3E <- df_3E[df_3E$Clusters %in% all_cl_genotypes, ]
p_3E <- ggplot(df_3E, aes(Clusters, Percentage, fill=Clusters)) + geom_bar(stat="identity") + ylim(0,100) + theme_classic() +
...
cat("  Fig 3H ...\n")
tryCatch({ DefaultAssay(RZ.RZK) <- "macs2"; RZ.RZK <- RegionStats(RZ.RZK, genome=linkage_genome)
    ...
    p_3H <- CoveragePlot(object=RZ.RZK, region="Wnt7b", features="Wnt7b", expression.assay="RNA", group.by="cl_genotype", extend.upstream=0, extend.downstream=0)
    ggsave(p_3H, filename=file.path(fig_dir,"fig_3H_Wnt7b_coverage.pdf"), width=8, height=8, dpi=300)
}, error=function(e) { cat(sprintf("    Fig 3H skipped: %s\n", e$message)); pdf(file.path(fig_dir,"fig_3H_Wnt7b_coverage.pdf")); plot.new(); title("Fig 3H - see log"); dev.off() })

cat("  Fig 3H statistics ...\n")
for(cl in 0:6) { wt_id <- paste0(cl,"_Wild_type"); mut_id <- paste0(cl,"_Mutant")
    if(!(wt_id %in% all_cl_genotypes) || !(mut_id %in% all_cl_genotypes)) next
    tryCatch({ wt_cells <- colnames(subset(RZ.RZK, subset=cl_genotype==wt_id)); mut_cells <- colnames(subset(RZ.RZK, subset=cl_genotype==mut_id))
...
for(region_str in cfg$wnt7b_regions) { tryCatch({
...
cat("  Fig S3B ...\n")
DefaultAssay(RZ.RZK) <- "RNA"; existing_wnt <- cfg$wnt_family_genes[cfg$wnt_family_genes %in% rownames(RZ.RZK)]
if(length(existing_wnt) > 0) { p_S3B <- VlnPlot(object=RZ.RZK, features=existing_wnt, group.by="kras_genotype")
